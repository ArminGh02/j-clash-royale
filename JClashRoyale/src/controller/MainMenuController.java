...
public class MainMenuController {
  ...
  @FXML
  private Label unableToPlaceCardLabel;

  @FXML
  void startGameVsEasyBot(ActionEvent event) {
    if (deck.size() == Deck.SIZE) {
      SoloGameController.getInstance().setRobotPlayer(new BeginnerRobot());
      SoloGameController.getInstance().setPersonDeck(deck);
      ViewManager.loadMapView();
    } else {
      DialogShower.show("Unable to Start Game", "You need to set your battle deck first!");
    }
  }
